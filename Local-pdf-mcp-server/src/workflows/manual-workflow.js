...
export function formatToolUsage(toolName = "", task = "") {
  ...
  lines.push("Canonical figure/visual-table flow: rebuild_figure_manifest -> search_figures -> get_figure_context_pack -> get_figure_image. Use get_figure_image transport=\"metadata\" to retrieve canonical_image_path/local_path, then open/attach the actual PNG as model vision input. mcp_image/image_url are experimental/client-dependent and not guaranteed to reach model vision input. image_path is a locator only; do not claim visual observation from path only.");
