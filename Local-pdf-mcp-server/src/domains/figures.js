...
const FIGURE_AGENT_INSTRUCTION = [
  ...
  "Use get_figure_image transport=\"metadata\" to retrieve canonical_image_path/local_path, then open or attach the actual PNG as model vision input. image_path is only a locator.",
  "mcp_image and image_url transports are experimental/client-dependent compatibility modes and are not proof that pixels reached the model; RICA/VS Code may reduce tool results to text-only.",
...
const VISUAL_CONTRACT = Object.freeze({
  ...
  image_transport_modes: ["metadata", "mcp_image", "image_url"],
  default_transport: "metadata",
  experimental_transports: ["mcp_image", "image_url"],
  transport_guarantee: "metadata_returns_paths_only; no_mcp_transport_guarantees_model_vision_input",
  ...
  forbidden_semantic_sources: ["page_text_before", "page_text_after", "ocr_text", "read_pdf_pages text", "search_pdf text", "renders/", "image_path string without loaded image content", "MCP image/data URI not actually attached to model vision input"]
});
...
export function buildFigureEvidenceContract(tool, filename, query, figures) {
  ...
  const needsVerification = [makeNeedsVerification({ item: "Visual content inside the actual figure/diagram", reason: "Visual content must be verified from actual image input; caption/text/OCR alone is insufficient and must not be treated as visual semantic truth.", suggestedTools: [`get_figure_image(filename="${filename}", figure_id="<figure-id>", transport="metadata")`, "open/attach canonical_image_path as actual model vision input", "read_pdf_pages(...) only as supporting text cross-check"] })];
  return makeEvidenceContract({ tool, filename, query, evidence, inference, needsVerification, warnings: ["Caption index is text-layer based; use it to locate visual pages, then verify canonical PNG pixels via actual image input. read_pdf_pages is supporting text evidence only."], recommendedNextTools: [`get_figure_context_pack(filename="${filename}", figure_id="<figure-id>")`, `get_figure_image(filename="${filename}", figure_id="<figure-id>", transport="metadata")`, "open/attach canonical_image_path as actual model vision input", `read_pdf_pages(filename="${filename}", start_page=<page>, end_page=<page>) # supporting text cross-check only`] });
}
